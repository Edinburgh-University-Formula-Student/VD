...
for i=1:Number_Of_Iterations
    clearvars -except Steering_Mode Travel_Mode Number_Of_Iterations Iteration_Step i
    %% VARIABLES
    chassis_height = 0.4; %m
    chassis_length = 1.5; %m
    chassis_width = 0.4; %m

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Inboard_TopBack_Pickup_UP = 0.1; %m
    Inboard_TopBack_Pickup_AFT = 0.25 + Iteration_Step*i; %m

    Inboard_TopFront_Pickup_UP = 0.1; %m
    Inboard_TopFront_Pickup_FOR = 0.25 + Iteration_Step*i; %m

    Inboard_BotBack_Pickup_UP = 0.15; %m
    Inboard_BotBack_Pickup_AFT = 0.3 + Iteration_Step*i; %m

    Inboard_BotFront_Pickup_DOWN = 0.18; %m
    Inboard_BotFront_Pickup_FOR = 0.3 + Iteration_Step*i; %m
    ...
    Rear_Wheelbase = 0.55; %m

    % -------------------------
    Tyre_Wall_Thickness = 2.5; Tyre_Wall_Thickness = Tyre_Wall_Thickness/39.37; %INCHES; m conversion
    Rim_Outer_Radius = 10; Rim_Outer_Radius = Rim_Outer_Radius/39.37; %INCHES; m conversion

    Wheel_width = 0.25; %m
    Rim_Thickness = 0.05; %m
    ...
    OutTop_Pickup_Dist = 0.1; %m
    OutBot_Pickup_Dist = 0.1; %m
    ...
    CASTER = 15; %DEG, Static Caster at ride height
    KINGPIN = 10; %DEG, Kingpin with respect to tyre rim
    Scrub_Radius = 0.01; %m, Scrub radius from steering axis to middle of tyre
    ...
    Bot_Length = sqrt( (Track_Width/2-chassis_width/2+BottomCamberBlueLength+OutBot_KingPin_Offset)^2  +  (abs( (Ride_Height + chassis_height/2 - Inboard_BotBack_Pickup_UP)  -  (Wheel_radius - OutBot_Pickup_Dist)*(1/cosd(CAMBER))  ))^2     );
    ...
    % LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL %
    Top_Length = sqrt( (Track_Width/2-chassis_width/2+TopCamberBlueLength+OutTop_KingPin_Offset)^2  +  (abs( (Ride_Height + chassis_height/2 + Inboard_TopBack_Pickup_UP)  -  (Wheel_radius + OutTop_Pickup_Dist)*(1/cosd(CAMBER))  ))^2     );
    ...
    TieRod_Length = sqrt( (TieRod2D_Length + TieRodToeRedLength)^2 + (TieRodToePurpleLength -TieRod_Pickup_FOR_AFT)^2 ); %-TieRodToeRedLength %- 0.0021;
    ...
    if Steering_Mode == true
        ...
        hold on
        subplot(1,2,1)
        plot(steering_toe, steering_camber)
        ...
        hold on
        subplot(1,2,2)
        plot(steering_toe, steering_caster)
        xlabel("Steering Angle")
        ylabel("Caster")
        hold off
    end
