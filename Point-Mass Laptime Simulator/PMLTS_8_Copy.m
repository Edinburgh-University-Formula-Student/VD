...
for i=1:length(v)
    Aero_Downforce = FzAero(Rho, Area, C_Lift, v(i)); % Aero Downforce (Total Force) (Affected by Velocity)
    Aero_Drag = FxAero(Rho, Area, C_Drag, v(i)); % Aero Drag (Total Forces) (Affected by Velocity)
    Roll_Drag = FxRoll(C_Roll, -1*FzMass(Mass, Track_Incl, Track_Bank) - Aero_Downforce); % Rolling Drag (Total Force) (Affected by Velocity)
    WeightOnDriven = (Factor_Drive * -1*(FzMass(Mass, Track_Incl, Track_Bank)) +  (-Factor_Aero*Aero_Downforce)) / Driven_Wheels; % Normal Forces On Driven Wheels (Affected by Velocity)
    ax_drag = (Aero_Drag + Roll_Drag + FxMass(Mass, Track_Incl))/Mass; % Total Drag Forces, in Acceleration form
    ay_max = (1/Mass) * (CF_Lat + D_muy*((Mass*9.81/Braking_Wheels)  -  (-1 * FzMass(Mass, Track_Incl, Track_Bank)   -  Aero_Downforce)/Braking_Wheels)) * (-1 * FzMass(Mass, Track_Incl, Track_Bank)   -  Aero_Downforce); % Maximum Lateral Acceleration Available From Tyres
    ...
    ax_tyre_max_dec = (-1/Mass) * (CF_Long + D_mux*( (Mass*9.81/Braking_Wheels) - (-1 * FzMass(Mass, Track_Incl, Track_Bank) - Aero_Downforce)/Braking_Wheels)) * (-1 * FzMass(Mass, Track_Incl, Track_Bank) - Aero_Downforce); % Maximum Longitudinal Deceleration Available From Tyres
