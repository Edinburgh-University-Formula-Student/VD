...
%     ax_tyre_max_dec = (1/Mass) * (CFlong + Dmux*(FzMass(Mass, trackIncl, trackBank) - Fz_Total_Force/braking_wheels)) * Fz_Total_Force;
% 
% 
% 
% 
...
%     % ax_acc = -1* (min(ax_tyre_acc, 10) - ax_drag) ; % limiting by engine power
%     ax_acc = 1* (ax_tyre_acc - ax_drag);
%     ax_dec = 1* (ax_tyre_max_dec*sqrt(1-(ay/ay_max).^2) - ax_drag) ; % friction ellipse
%     % saving GGV map
...
for i=1:length(corners)
    ...
    for j=1:length(track1)
        ...
        %% TODO: Fix?
        % if GGVaccIndex >= (vMax/dv)
        %     GGVaccIndex = (vMax/dv)-1;
        % end
        % 
        % if GGVdecIndex >= (vMax/dv)
        %     GGVdecIndex = (vMax/dv)-1;
        % end
        %%

        axDec = abs(GGV(GGVdecIndex, LongitudinalDecel, 1));
        axAcc = abs(GGV(GGVaccIndex, LongitudinalAccel, 1));
        Ay = abs(GGV(GGVdecIndex, LateralIndexPositive, 2));
        
        % disp("Velocity: " + old_decel + "GGV Index: " + GGVdecIndex + " Ay: " + ay)
        ...
        if track1(1,dec_index) == inf% || track1(1,dec_index) == 0 % During Straights
            ...
            elseif new_dec < old_decel
                %% TODO: Function never called VVV (Potentially fixed with GGV fixes?)
        ...
        % 
        %     if old_accel == 0
        %         if acceleration_speed(previous_index) ~= 0
        %             acceleration_speed(1, acc_index) = min(apex_accel, sqrt((acceleration_speed(1, previous_index)^2) + 2 * axAcc * deltaX )); % Determine whether it is possible to acheive maximum apex speed given previous speed, or corner is flat out
        %         else
        %             acceleration_speed(1, acc_index) = apex_accel;
        %         end
