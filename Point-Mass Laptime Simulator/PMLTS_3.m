...
for i=1:length(v)
    ...
    ay_max = (1/Mass) * (CFlat + Dmuy*(FzMass(Mass, trackIncl, trackBank) - (Fz_Total_Force/4))) * Fz_Total_Force/4;

    % max long acc available from tyres
    ax_tyre_max_acc = (1/Mass) * (CFlong + Dmux*(FzMass(Mass, trackIncl, trackBank) - Fz_Total_Force/4)) * Fz_Total_Force;
    ...
    % ax_acc = (min(ax_tyre_acc,25) + ax_drag) ; % limiting by engine power
...
subplot(1,2,1);
title('GGV Map')
surf(GGV(1:200,:,2),GGV(1:200,:,1),GGV(1:200,:,3))
% grid on
...
for i=1:length(corners)
    ...
    for j=1:length(track1)
        ...
        decIndex = round((old_decel * (1/dv) + 1));
        accIndex = round((old_accel * (1/dv) + 1));
        % decIndex = 250;
        % accIndex = 250;
        ...
        axDec = abs(GGV(decIndex, LongitudinalDecel, 1));
        axAcc = abs(GGV(accIndex, LongitudinalAccel, 1));
        % ay = abs(GGV(decIndex, LateralIndexPositive, 2));
        %% TODO
        ay = axAcc; 
        %%
        ...
        % axDec = max(abs(GGV(decIndex, :, 1)));
        % axAcc = max(abs(GGV(accIndex, :, 1)));
        % ay = max(abs(GGV(decIndex, :, 2)));
